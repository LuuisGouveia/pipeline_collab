...
def extract_data():
    ...
    lists_path = os.path.join(project_root, "data", "lists")  # Pasta contendo arquivos tabulares de listas
    notes_path = os.path.join(project_root, "data", "notes")    # Pasta contendo arquivos tabulares de notas
    pdf_path = os.path.join(project_root, "data", "pdf")       # Pasta contendo arquivos PDF a serem processados
    ...
    df_pdfs = extract_pdf(pdf_path)
    
    # Consolida todos os DataFrames em um único DataFrame unificado
    all_dfs = [df for df in [df_tables, df_notes, df_pdfs] if df is not None and not df.empty]
